...
class LogRecord implements ArrayAccess
{
    ...
    public function &offsetGet(mixed $offset): mixed
    {
        ...
        if ($offset === 'level_name') {
            ...
            $copy = is_object($this->level) ? $this->level->getName() : (string) $this->level;

            return $copy;
        }
        if ($offset === 'level') {
            // avoid returning readonly props by ref as this is illegal
            $copy = is_object($this->level) ? $this->level->value : (int) $this->level;
    ...
    public function toArray(): array
    {
        return [
            ...
            'level' => is_object($this->level) ? $this->level->value : (int) $this->level,
            'level_name' => is_object($this->level) ? $this->level->getName() : (string) $this->level,
